...
pub(crate) async fn handle_command(
    ...
    match cmd {
        SalaryCommands::Add {
            ...
            let all_params_provided = date.is_some() && amount.is_some() && currency.is_some() && frequency.is_some();

            // Get effective date interactively if not provided
            let effective_date = match date {
                Some(d) => {
                    NaiveDate::parse_from_str(d, "%Y-%m-%d")
                        .map_err(|e| color_eyre::eyre::eyre!("Invalid date format: {}. Use YYYY-MM-DD", e))?
                }
                None => {
                    let today = Local::now().date_naive();
                    let default_date = today.format("%Y-%m-%d").to_string();

                    let date_str = Text::new(&format!("Effective date (YYYY-MM-DD, press Enter for today {}):", default_date))
            ...
            let amount = match amount {
                Some(a) => *a,
                None => {
                    CustomType::<f64>::new("Annual salary amount (e.g., 50000 for £50,000):")
            ...
            let notes = match notes {
                Some(n) => Some(n.clone()),
                None => {
                    Text::new("Notes (optional - press Enter to skip):")
                        .prompt_skippable()
                        .map_err(|e| color_eyre::eyre::eyre!("Failed to get input: {}", e))?
                }
            };
            ...
            match repo.create_salary(new_salary).await {
                Ok(salary) => {
                    let currency_symbol = if salary.currency == "GBP" { "£" } else { &salary.currency };
                    println!("✓ Added salary entry effective {} with amount {} {}",
        ...
        SalaryCommands::Current => {
            match repo.get_current_salary().await {
                Ok(Some(salary)) => {
                    let currency_symbol = if salary.currency == "GBP" { "£" } else { &salary.currency };
                    println!("Current salary (effective {}):", salary.effective_date);
                    println!("  Amount: {} {}", salary.amount.format_currency(currency_symbol), salary.frequency);
                ...
                None => {
                    match repo.list_salaries().await {
                        Ok(salaries) if !salaries.is_empty() => {
                            let salary_options: Vec<String> = salaries.iter()
                                .map(|s| {
                                    let currency_symbol = if s.currency == "GBP" { "£" } else { &s.currency };
                                    let notes_preview = s.notes.as_ref()
                                        .map(|n| format!(" - {}", n))
                            ...
                            let selected = Select::new("Select salary entry to delete:", salary_options)
                                .prompt()
                                .map_err(|e| color_eyre::eyre::eyre!("Failed to get input: {}", e))?;
                            ...
                            selected.split(':').next()
                                ...
                                .ok_or_else(|| color_eyre::eyre::eyre!("Failed to parse salary ID"))?
                        }
                        Ok(_) => {
                ...
                Ok(Some(salary)) => {
                    if !force {
                        ...
                        let confirmed = Confirm::new(&format!(
                            ...
                            id, salary.effective_date, salary.amount.format_currency(currency_symbol)
                        ))
                            .with_default(false)
                            .prompt()
                            .map_err(|e| color_eyre::eyre::eyre!("Failed to get input: {}", e))?;
